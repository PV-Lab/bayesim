...
2) there is significantly less uncertainty that all regions of non-negligible probability are quickly detected, since the coarsest iteration tends to identify all “hotspots” immediately.
...
.. A very common approach to multidimensional Bayesian parameter estimation involved Monte Carlo sampling rather than sampling on a grid as we do here. In general, such approaches scale very well to larger numbers of dimensions and hence may have great appeal. However, we chose to use a gridded sampling approach (with successive subdivisions of high-probability boxes) because our applications of interest involve models that can be computationally expensive to evaluate. In a Monte Carlo approach, parameter space points that are identical or very close to each other would be sampled many times, wasting computational effort, when they only need to be sampled once to compute their likelihood. **Thus,** ``bayesim`` **shines in situations where the computational effort required to evaluate the likelihood is large, as when the data models are sophisticated numerical solvers.**
...
In addition, it's important to note that while the subdivided grid approach (aided at the initial coarse stages by calculating :ref:`model uncertainty <model-uncertainty>`) can make somewhat higher dimensions accessible than a fully "brute-force" grid sampling could, computational load for the initial (pre-subdivision) set of calculations will still scale exponentially in the number of fitting parameters, and thus ``bayesim`` will struggle in high dimensions.
